...
deploy:
	gcloud --project=$(GOOGLE_CLOUD_PROJECT) functions deploy $(NAME) --entry-point $(FUNCTION_ENTRYPOINT) --max-instances 1 --set-env-vars=$(ENV_VARS) --trigger-http --runtime ruby32 --memory=128M --timeout=20 --region $(REGION) --allow-unauthenticated
	@echo $(INVOKE_URL)
