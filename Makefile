...
deploy:
	gcloud --project=$(GOOGLE_CLOUD_PROJECT) functions deploy $(NAME) --entry-point $(FUNCTION_ENTRYPOINT) --max-instances 1 --set-env-vars=$(ENV_VARS) --trigger-http --runtime ruby30 --memory=128M --timeout=20 --region $(REGION) --allow-unauthenticated --no-gen2
	@echo $(INVOKE_URL)
